...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            levelEdge = PgNode_a(action)
            if levelEdge.prenodes.issubset(self.s_levels[level]):
                level_actions.add(levelEdge)
                for levelNode in self.s_levels[level]:
                    levelNode.children.add(levelEdge)

        self.a_levels.insert(level, level_actions)
    ...
    def add_literal_level(self, level):
        ...
        for parent_edge in self.a_levels[level -1]:
            for result_node in parent_edge.effnodes:
                literals.add(result_node)
                parent_edge.children.add(result_node)
                result_node.parents.add(parent_edge)

        self.s_levels.insert(level, literals)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        check1 = len([a for a in  node_a1.action.effect_add if a in node_a2.action.effect_rem])

        check2 = len([a for a in node_a2.action.effect_add if a in node_a1.action.effect_rem])
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        check1 = len([a for a in node_a1.action.effect_rem if a in node_a2.action.precond_pos])

        check2 = len([a for a in node_a2.action.effect_rem if a in node_a1.action.precond_pos])
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        visistedStates = set()
        #print("Goal length", len(self.problem.goal))
            ...
            for g in self.problem.goal:
                for state in levelStates:
                    if (g == state.symbol and state not in visistedStates):
                        ...
                        break
                    #visistedStates.add(state.symbol)
        return level_sum-1